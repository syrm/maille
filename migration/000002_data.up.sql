...
VALUES ('Assets'::account_type, 'Bank:Checking', 'Checking', '💰', '#a8c8f0'),
       ...
       ('Income'::account_type, 'Salary:Fixed', 'Salaire', '👷‍♂️', '#a8f0c8'),
       ('Income'::account_type, 'Salary:Variable', 'Salaire', '👷‍♂️', '#a8f0c8')
;
...
VALUES ('payee contains "AMAZON"', (SELECT id FROM account WHERE name = 'Shopping:Other')),
       ...
       ('payee contains "VIR SEPA SALAIRE ENTREPRISE A"', (SELECT id FROM account WHERE name = 'Salary:Fixed')),
       ('payee contains "VIR SEPA SALAIRE FREELANCE"', (SELECT id FROM account WHERE name = 'Salary:Variable'))
