...
let init = () => {
  // ── Bar
  let _bar = switch document->Core.getElementById("bar") {
  | Value(element) =>
    Bar.make(
      element,
      [
  ...
  // ── Line area
  let _line = switch document->Core.getElementById("line") {
  | Value(element) =>
    Line.make(
      element,
  ...
  // ── Pie
  let _pie = switch document->Core.getElementById("pie") {
  | Value(element) =>
    Pie.make(
      element,
      [
  ...
  // ── Multi line
  let _multi = switch document->Core.getElementById("multi") {
  | Value(element) =>
    Line.make(
      element,
      [
  ...
  // ── Donut
  let _donut = switch document->Core.getElementById("donut") {
  | Value(element) =>
    Pie.make(
    element,
    [
